...
// 静的なHTMLテキストはja/en各ページ側で直接翻訳しているが、JSが実行時に生成する
// テキスト（アラート・ボタン文言の切替・共有テキスト等）はこの辞書で切り替える。
...
const I18N = {
    ...
    modeDescMorseBoard: {
        ja: `コールサインとRSTが表示される。<br>Morse BoardをPCに接続し、表示された文字をパドルで入力。<br>キーボードとして入力された英数字を、そのまま解答として判定します。`,
        en: `A callsign and RST report are shown.<br>Connect a Morse Board to your PC and key the displayed characters on the paddle.<br>Alphanumeric characters received as keyboard input are judged directly as your answer.`
    },
    modeDescTouch: {
        ja: `コールサインとRSTが表示される。<br>スマホ・タブレットは画面の「トン/ツー」をタップして送信。<br>キーボードでも操作可（右記参照）。<br>長押しで連続、両方同時押しで交互（アイアンビック）。離すと自動判定。`,
        en: `A callsign and RST report are shown.<br>On phone/tablet, tap the on-screen "DIT/DAH" buttons to send.<br>Keyboard input also works (see right).<br>Hold for repeats, hold both together to alternate (Iambic). Release to auto-judge.`
    },
    modeDescDesktop: (dotLabel, dashLabel) => ({
        ja: `コールサインとRSTが表示される。<br>${dotLabel}キー長押し＝連続短点（・・・）、${dashLabel}キー長押し＝連続長点（－－－）<br>両方同時押しで交互（アイアンビック）。離すと自動判定。`,
        en: `A callsign and RST report are shown.<br>Hold ${dotLabel} = repeated dits (・・・), hold ${dashLabel} = repeated dahs (－－－)<br>Hold both together to alternate (Iambic). Release to auto-judge.`
    }),
...
// コンテストナンバーの「カットナンバー」表記: 1→A(・－)、0→O(－－－)/T(－)。
// 送信モードでは本来の数字の符号に加えて、これらのカットナンバーでも正解とする
// （コールサイン打鍵時は対象外。コンテストナンバーのみ）。表示は常に本来の符号。
const CUT_NUMBER_ACCEPT = { '1': ['A'], '0': ['O', 'T'], '9': ['N'] };
...
let nextTxType = 'CALLSIGN';
let txWordIsCallsign = true; // 現在のtxWordがコールサインかどうか（カットナンバーはコンテストナンバーのみに適用）
let txCharTimer = null;
let txMistakeCounts = {}; // 文字ごとの打ち間違え回数（リザルト画面の「ミスが多かった文字」表示用）
let TX_CHAR_GAP = 280;
...
function generateRST() {
    ...
    return '5NN' + areas[Math.floor(Math.random() * areas.length)] + ['H','M','L','P'][Math.floor(Math.random()*4)];
}
...
function nextTxWord() {
    txWordIsCallsign = (nextTxType === 'CALLSIGN');
    const raw = txWordIsCallsign ? generateCallsign() : generateRST();
    nextTxType = txWordIsCallsign ? 'RST' : 'CALLSIGN';
    txWord = raw.split('');
...
function updateTxBeginnerDisplay() {
    ...
    // 表示は常に本来の1・0の符号のまま（カットナンバーは正解判定でのみ別解として許容）
...
function evaluateTxChar(decoded) {
    ...
    // コールサインの1・0は本来の符号のみ正解（カットナンバー不可）。
    // コンテストナンバーの1・0は本来の符号に加え、カットナンバー（1→A、0→O/T）も正解とする。
    const cutAlts = (!txWordIsCallsign && CUT_NUMBER_ACCEPT[expected]) || [];
    const isCorrect = (decoded === expected) || cutAlts.includes(decoded);
    if (isCorrect) {
        txCharIndex++;
        score++;
        ...
        txMistakeCounts[expected] = (txMistakeCounts[expected] || 0) + 1;
...
function renderMistakeBreakdown() {
    const area = document.getElementById('tx-mistake-area');
    const list = document.getElementById('tx-mistake-list');
    if (!area || !list) return;
    const entries = Object.entries(txMistakeCounts).sort((a, b) => b[1] - a[1]);
    if (entries.length === 0) {
...
async function autoRegister(retryCount = 0) {
    ...
    if (!_registeredId) {
        try {
            const res = await fetch('https://api.jl1gix.com/api/scores', {
                ...
                body: JSON.stringify({ nickname: 'noname', score, mode: 'TX', is_beginner: beginnerMode ? 1 : 0, minutes })
            });
    ...
    let pct = '－';
    try {
        const rankRes = await fetch(`https://api.jl1gix.com/api/scores/rank?score=${score}&mode=TX&minutes=${minutes}`);
